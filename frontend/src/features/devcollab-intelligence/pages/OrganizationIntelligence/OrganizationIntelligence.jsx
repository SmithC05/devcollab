...
export default function OrganizationIntelligence() {
  ...
  // Phase 3: MEMBER_UNAVAILABLE banner state
  ...
  useEffect(() => {
    ...
    const handleEngineEvent = (e) => {
      ...
      if (payload.event_type === 'DECISION_POINT_CREATED') {
        setDecisionPoint(payload);
        // Also show the MEMBER_UNAVAILABLE banner
        if (payload.affected_member) {
          setUnavailableBanner(payload);
        }
      }
      if (payload.event_type === 'MEMBER_UNAVAILABLE') {
        setUnavailableBanner(payload);
      }
      if (payload.event_type === 'TASK_REASSIGNED' || payload.event_type === 'PRESENCE_RESTORED') {
        // Refetch is already called above
