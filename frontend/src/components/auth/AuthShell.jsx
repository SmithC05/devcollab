...
function LeftContent({ mode, onSwitchToLogin, onSwitchToRegister }) {
  ...
  return (
    <div className="h-full flex items-center justify-center" style={{ padding: '56px 78px 55px 78px' }}>
...
export default function AuthShell({ initialMode = 'login' }) {
  ...
  const [curtainAnim, setCurtainAnim] = useState({ key: 0, from: '-100%', to: '-100%' });
  ...
  return (
    <div
      ...
      className="
        ...
        border border-[#242424] dark:border-[#242424]
        rounded-[30px]
        shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_32px_64px_rgba(0,0,0,0.7)]
      "
      style={{
        maxWidth: 1200,
    ...
    >
      ...
      <div className="grid grid-cols-2 h-full">
        ...
        <div
          ...
          style={{ background: '#0B0B0B', borderRight: '1px solid #1D1D1D' }}
        >
          <LeftContent
            mode={displayMode}
      ...
      */}
      <motion.div
        key={curtainAnim.key}
        className="absolute inset-y-0 left-0 w-full z-50 pointer-events-none"
        style={{
          background: 'linear-gradient(135deg, #0f0f0f 0%, #111111 60%, #0d0d0d 100%)',
